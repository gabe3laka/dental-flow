...
export default function Progress() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const mapped: ScanRecord[] = (scansData || []).map((s: Record<string, unknown>) => ({
          id: s.id as string,
          submitted_at: s.submitted_at as string,
          quality_score: (s.quality_score as number | null) ?? null,
          detection_tags: Array.isArray(s.detection_tags) ? (s.detection_tags as string[]) : null,
          sent_to_doctor: (s.sent_to_doctor as boolean | null) ?? false,
          ai_analysis: s.ai_analysis,
          pointcloud_url: (s.pointcloud_url as string | null) ?? null,
          processing_status: (s.processing_status as string | null) ?? null,
          scan_type: (s.scan_type as string | null) ?? (s.source as string | null) ?? null,
        }));
        setAllScans(mapped);
  ...
  return (
    <div className="min-h-screen bg-background px-5 py-8 max-w-[480px] mx-auto pb-24">
      ...
      {/* Patient's own 3D map — point cloud from latest scan */}
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <span className="mono-label text-muted-foreground">YOUR 3D MAP</span>
          {latestScan?.scan_type && (
            <span className="mono-label text-[9px] px-1.5 py-0.5 rounded-full bg-primary/15 text-primary">
              {latestScan.scan_type.toUpperCase()}
            </span>
          )}
        </div>
        <div className="rounded-card overflow-hidden bg-card border border-border dark">
          {latestScan?.pointcloud_url ? (
            <PointCloudViewer plyUrl={latestPointcloudUrl} height={320} />
          ) : !latestScan ? (
            <div className="h-[320px] bg-black flex flex-col items-center justify-center gap-3 text-center px-6">
              ...
              <span className="mono-label text-white/60 text-[10px]">NO SCAN YET</span>
              <p className="text-white/40 text-xs max-w-[220px]">
                Take your first scan to see your own 3D map here.
              </p>
              <Button
          ...
          {latestScan && (
            <div className="px-5 pb-4 pt-3" style={{ background: "hsl(var(--card))" }}>
              ...
              <p className="mono-label text-muted-foreground text-[10px] mt-2">
                Latest scan · {differenceInDays(new Date(), new Date(latestScan.submitted_at))} days ago
              </p>
              <Button
                onClick={() => navigate("/patient/scan")}
                className="w-full rounded-pill mono-label bg-primary text-primary-foreground mt-3"
              >
                <Camera className="w-3.5 h-3.5 mr-1" />
                Take New Scan
              </Button>
