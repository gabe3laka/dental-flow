...
import { Send, Paperclip, MessageCircle, Search, Info } from "lucide-react";
...
export default function PatientChat() {
  ...
  const [activeTab, setActiveTab] = useState<TabId>(navState.activeTab === "find" ? "find" : "messages");
  ...
  // Context from detection navigation
  const fromDetection = navState.fromDetection;
  const prefilterSpecialty = navState.prefilterSpecialty;

  useEffect(() => {
    if (!user) return;
    (async () => {
      const { data: patient } = await supabase
        .from("patients").select("id, assigned_doctor_id").eq("user_id", user.id).maybeSingle();

      if (patient?.assigned_doctor_id) {
        setDoctorId(patient.assigned_doctor_id);
        const { data: profile } = await supabase
          .from("profiles").select("full_name").eq("user_id", patient.assigned_doctor_id).maybeSingle();
        ...
        await supabase.from("messages")
          ...
          .eq("receiver_id", user.id).eq("sender_id", patient.assigned_doctor_id).is("read_at", null);
      }

      if (patient?.id) {
        const { data: scan } = await supabase
          .from("scans").select("detection_tags").eq("patient_id", patient.id)
  ...
  const sendMessage = async () => {
    ...
    try {
      const { error } = await supabase.from("messages").insert({
        sender_id: user.id, receiver_id: doctorId, content: input.trim(),
      });
    ...
    <div className="min-h-screen bg-background flex flex-col max-w-[480px] mx-auto pb-24">
          ...
          <div ref={scrollRef} className="flex-1 overflow-auto px-5 py-4 space-y-2">
            {!doctorId && (
              <div className="flex flex-col items-center justify-center py-16 px-4">
                ...
                <h2 className="font-display text-base font-medium text-foreground mb-2">No doctor connected yet</h2>
                ...
                <button onClick={() => setActiveTab("find")} className="rounded-pill px-5 py-2 mono-label border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition">
                  Find a Doctor →
                </button>
              </div>
            )}
                ...
                <div key={msg.id}>
                  ...
                  <div className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[75%] px-4 py-2.5 text-sm ${
                      isOwn ? "bg-primary text-primary-foreground rounded-[18px_18px_4px_18px]" : "bg-secondary text-secondary-foreground rounded-[18px_18px_18px_4px]"
                    }`}>
                      {msg.content}
                    </div>
                  </div>
            ...
            <div className="px-5 py-4 border-t border-border">
              <div className="flex items-center gap-2 bg-card rounded-pill border border-border px-4 py-2">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={async (e) => {
                    ...
                    try {
                      ...
                      await supabase.from("messages").insert({
                        sender_id: user.id, receiver_id: doctorId,
                        content: "📎 Sent an image", attachment_url: urlData.publicUrl, message_type: "scan_attachment" as any,
                      });
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()}
                  placeholder="Type a message..." className="flex-1 bg-transparent text-sm font-body outline-none placeholder:text-muted-foreground" />
                <button onClick={sendMessage} disabled={!input.trim() || sending}
                  className="w-8 h-8 rounded-full bg-primary flex items-center justify-center disabled:opacity-50" aria-label="Send message">
      ...
      {activeTab === "find" && (
        <div className="flex-1 overflow-auto px-5 py-5">
          {fromDetection && (
            <div className="flex items-start gap-2 rounded-card bg-primary/5 border border-primary/20 p-3 mb-4">
              <Info className="w-4 h-4 text-primary mt-0.5 flex-shrink-0" />
              <p className="text-xs text-foreground/80">
                Based on your scan results, we recommend seeing a specialist. Browse doctors below to find the right match.
              </p>
            </div>
          )}
          {detectionTags.length > 0 && !fromDetection && (
            <p className="text-xs text-muted-foreground mb-4 italic">
              Showing recommendations based on your latest scan results.
            </p>
          )}
          <DoctorMarketplace mode="grid" detectionTags={detectionTags} onAssign={() => setActiveTab("messages")} />
        </div>
      )}
